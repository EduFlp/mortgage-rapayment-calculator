...
document.querySelector('#calculateBtn').addEventListener('click', (event) => {
    ...
    document.querySelector('.errorField').forEach(errorField => {
        errorField.style.display = 'none';
    });
    document.querySelector('.inputBox').forEach(inputBox => {
    ...
    const repayment = (document.querySelector('input[name = "mortgageType"]: checked') !== null) ? 
    document.querySelector('input[name="mortgageType"]:checked').value : 'null';
    if(amount === '' || amount <= 0) {
    ...
    if (errors.length < 1) {
        ...
        if(+repayment === 1) {
            ...
            document.getElementById('totalAmount').innerHTML = '&pound;' + new Intl.NumberFormat('en-US', {minimumIntegerDigits: 2, maximumFractionDigits: 2}).format(+totalAmount);
        }
        else if (+repayment === 2) {
